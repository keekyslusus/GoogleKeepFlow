...
logger = setup_worker_logger("pin_worker", plugindir)
...
def main():
    ...
    try:
        ...
        title = "Note Pinned" if pinned else "Note Unpinned"
        logger.info("Note pin state changed: pinned=%s", pinned)
        logger.info("%s: id=%s", title, note_id)
    except Exception as exc:
        logger.error("Failed to update pin state: %s: %s", type(exc).__name__, exc)
        show_notification(
